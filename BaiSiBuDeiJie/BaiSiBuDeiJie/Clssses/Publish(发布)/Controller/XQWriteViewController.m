...
- (void)viewDidLoad {
    ...
    int coloumn = 3;
    CGFloat btnWH = 120;
    CGFloat margin = (self.view.ldc_width - coloumn * btnWH) / (coloumn + 1);
    CGFloat x = 0;
    CGFloat y = 0;
    int curL = 0;
    int curR = 0;
    ...
    for (int i = 0; i < btnArray.count; i++) {
        ...
        //添加按钮时,把所有的按钮移动底部
        btn.transform = CGAffineTransformMakeTranslation(0, LDCScreenH);
        //监听按钮点击
    ...
    CGFloat btnH = 49;
    bottomV.frame = CGRectMake(0, LDCScreenH - btnH, LDCScreenW, btnH);
    ...
    closeBtn.frame = CGRectMake((bottomV.ldc_width - closeBtnW) * 0.5, (bottomV.ldc_height - closeBtnW) * 0.5, closeBtnW, closeBtnW);
...
- (void)colse {
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        if ([view isKindOfClass:[XQCenterTitleBtn class]]) {
            ...
            [UIView animateWithDuration:0.5 delay: i * 0.05 usingSpringWithDamping:0.9 initialSpringVelocity:0 options:UIViewAnimationOptionCurveLinear animations:^{
                //把所有的按钮移动底部
                view.transform = CGAffineTransformMakeTranslation(0, LDCScreenH);
            } completion:nil];
        }
    }
